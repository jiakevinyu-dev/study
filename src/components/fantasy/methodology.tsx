...
const ITEMS = [
  ...
  },
  {
    ...
    body:
      "Sleeper's player list keeps long-retired players on file indefinitely and doesn't always clear a stale team — this tool requires Sleeper's active flag before a player is shown, and separately excludes anyone whose status is explicitly 'Retired,' since active isn't reliably flipped for every retired player either (Tom Brady, notably, has slipped through both before). If anything wrong still slips through, hit the ⊘ next to any row to pull him out of the pool entirely (recomputes replacement levels without him). Excluded players are listed at the bottom of the table with one click to restore. Separately, League Settings has a relevance cutoff (default: ADP/search_rank worse than 250) that drops the deep, practice-squad-tier tail of Sleeper's full player dump from the board outright — it doesn't just declutter the table, it's the backstop that catches anyone the flag/status checks miss, and keeps that long tail from skewing tiers and replacement levels for real starter-quality players. Anyone already on your roster or off the board stays visible regardless of the cutoff; clear the field to show everyone.",
  },
];
