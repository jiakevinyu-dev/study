...
const ITEMS = [
  {
    title: "Points",
    body:
      "If a player has a real stat-line projection imported (CSV/manual), points are the exact dot product of that stat line with your current scoring settings — change PPR value, TE premium, or any weight and every player's points update immediately. Without one, points are estimated from a position-specific decay curve (marked with * in the table) keyed to the real numeric ADP/search_rank gap behind that position's own #1 player — not an ordinal '1st, 2nd, 3rd...' position-rank count. That distinction matters: a shallow position like TE has maybe 10 truly relevant options, so a few ordinal spots there can be tens of real draft picks apart, while the same ordinal gap in a deep position (RB/WR) is only a handful of picks — decaying by position-rank count exaggerated shallow-position gaps into cliffs the real market doesn't see. The curve's ceiling constants are calibrated from real evidence, not guessed in a vacuum — RB's is set from a real final-2025 stat line the user provided, which showed the actual #1 fantasy scorer that season was a bell-cow RB outscoring every real QB, WR, and TE. That's a one-time, aggregate calibration of the shared curve, deliberately not a per-player injection of one season's real (noisy — injuries, TD variance, rookie randomness) outcome into individual players' points. This is still a shape approximation for sorting, not a projection.",
  },
  {
