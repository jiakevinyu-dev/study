...
const ITEMS = [
  {
    title: "Points",
    body:
      "If a player has a real stat-line on file, points are the exact dot product of that stat line with your current scoring settings — change PPR value, TE premium, or any weight and every player's points update immediately. Two sources feed a real stat line: your own CSV/manual import, and (new) a bundled real final 2025 box score for ~213 fantasy-relevant players, used as a same-season baseline when Sleeper hasn't given you anything better — see 'Real 2025 stats' below. Without either, points are estimated from a position-specific decay curve (marked with * in the table) keyed to the real numeric ADP/search_rank gap behind that position's own #1 player — not an ordinal '1st, 2nd, 3rd...' position-rank count. That distinction matters: a shallow position like TE has maybe 10 truly relevant options, so a few ordinal spots there can be tens of real draft picks apart, while the same ordinal gap in a deep position (RB/WR) is only a handful of picks — decaying by position-rank count exaggerated shallow-position gaps into cliffs the real market doesn't see. This is still a shape approximation for sorting, not a projection — real stats are always preferred where available.",
  },
  {
    title: "Real 2025 stats — why RB now shows its real cliff",
    body:
      "The ADP-decay curve above is a smooth, single-shape approximation — it can't know a position's real value curve is uneven. RB is the clearest case: 2025's real numbers show an enormous gap between the top ~10 bell-cow backs and everyone after (a well-known 'RB dead zone'), while QB in a 2QB/superflex-relevant range is comparatively flat. A hand-tuned smooth curve can't reproduce an uneven cliff like that, which was quietly flattening RB's real early-round premium relative to other positions. So every Sleeper-synced player who appears in a bundled real final-2025 stat line (receptions, yards, TDs, etc. — pasted in directly, ~213 players) gets that real line instead of the estimate, scored the normal way: an exact dot product with your scoring settings, not a curve. It's last season's real outcome used as a baseline, not a 2026 forecast — an explicit, disclosed choice, not a claim about what will happen this year, so it won't reflect this year's injuries, role changes, or rookies with no 2025 log. Anyone not in it (rookies, post-trade role changes, deep waiver names) still falls back to the decay curve. A CSV/manual projection you've imported always wins over this bundle.",
  },
  {
  ...
  {
    title: "Position caps — the strongest lineup is mostly RB/WR",
    body:
      "League Settings has a max roster spots per position — QB and TE both default to 2 (RB/WR uncapped), so a well-built roster ends up with 1-2 QBs, 1-2 TEs, and everything else at RB/WR, which is where a typical redraft roster's real depth and flex value live. Once you've hit a cap, that position stops appearing in the Recommended Pick list entirely — not deprioritized, excluded — because marginal value and scarcity drop-off only ever know 'this player has some value somewhere,' never 'my roster will never actually carry a 3rd one of these regardless.' This is also the mechanism behind 'a deep position's 2nd guy should come late': with TE capped at 2 and one already rostered, the model only ever recommends TE2 when he's genuinely your best remaining marginal value at any position — which, because TE runs deep, tends not to happen until the shallower positions have thinned out. The full table and Mine/Taken still work normally past a cap; it only affects what's recommended for you.",
  },
  {
  ...
  {
    title: "Real reference list — the actual fix for bad Sleeper data",
    body:
      "Sleeper's player list keeps long-retired players on file indefinitely, doesn't always clear a stale team, and its active flag and search_rank both proved unreliable in practice — a clearly-retired player (Tom Brady) got through both. So every Sleeper-synced player now also has to appear by name on a bundled real-world reference list — a human-curated 2026 superflex/2QB PPR consensus ranking (271 players, FantasyPros) — or he's left off the board entirely. That's a hard whitelist against real judgment, not another guess at a Sleeper field to trust. It also doubles as a real-ADP source: any Sleeper player the list covers gets its rank as his ADP automatically on sync, feeding real numbers into Value vs ADP and the points estimate instead of Sleeper's own proxy. It's a snapshot, though — it'll drift over the season (trades, cuts, waiver breakouts), and CSV-imported/manual players (the sample dataset, your own rankings import) were never part of Sleeper's noisy dump and skip this check entirely. The numeric relevance cutoff below still applies on top for everyone, including trimming this list itself if you set it under 271. If anything still looks wrong, hit the ⊘ next to any row to pull him out of the pool entirely (recomputes replacement levels without him) — excluded players are listed at the bottom of the table with one click to restore.",
  },
];
